...
- **Phase 1**: the real data series. I picked the pairings: DW-NOMINATE polarization for Huntington, TFP growth for Kondratiev, WID top-1% wealth share for Turchin, conflict deaths (log) for Khaldun, US share of world GDP for Dalio, V-Dem liberal-democracy for Strauss-Howe. (Perez has no paired data this round - the techno-economic paradigm story is harder to reduce to a single century-long series.) The model fetched, parsed, and wired up the CSV ingestion. I read each series myself and rejected one fit that felt like cherry-picking.
